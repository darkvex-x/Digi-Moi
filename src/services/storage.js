...
const readDB = () => {
  try {
    ...
    console.error("Local Storage Data Corruption. Resetting to defaults.", error);
    return defaultDB;
  }
};
...
export const StorageService = {
  ...
  getEvents: () => {
    return new Promise((resolve) => {
      ...
      resolve([...db.events].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
    });
  },
  ...
  deleteEvent: (id) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (db.events.length === initialLen) return reject(new Error("Event not found"));
  ...
  getEntries: (eventId = null) => {
    return new Promise((resolve) => {
      ...
      resolve([...res].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
    });
  },
  ...
  createEntry: (data) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!data.amount || data.amount <= 0) return reject(new Error("Valid amount is required"));
        ...
        if (eventIndex === -1) return reject(new Error("Target Event does not exist"));

        // Generate Receipt Number automatically (Max current + 1)
        const eventEntries = db.entries.filter(e => e.eventId === data.eventId);
  ...
  updateEntry: (id, updates) => {
    return new Promise((resolve, reject) => {
      try {
        ...
        const newAmount = updates.amount !== undefined ? Number(updates.amount) : oldEntry.amount;
        ...
        if (amountDiff !== 0) {
          const eventIndex = db.events.findIndex(e => e.id === oldEntry.eventId);
          if (eventIndex !== -1) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!parsed || typeof parsed !== 'object') {
          return reject(new Error('Invalid backup: Payload must be a JSON object.'));
        }
        if (!parsed.settings || typeof parsed.settings !== 'object') {
          return reject(new Error('Malformed backup: Settings block is missing or invalid.'));
        }
        if (!Array.isArray(parsed.events)) {
          return reject(new Error('Malformed backup: Events list must be an array.'));
        }
        if (!Array.isArray(parsed.entries)) {
          return reject(new Error('Malformed backup: Entries list must be an array.'));
        }

        // 2. Settings Validation (No HTML escaping)
        const cleanSettings = {
          businessName: String(parsed.settings.businessName || defaultDB.settings.businessName),
          receiptPrefix: String(parsed.settings.receiptPrefix || defaultDB.settings.receiptPrefix),
          currency: String(parsed.settings.currency || defaultDB.settings.currency),
          paperWidth: String(parsed.settings.paperWidth || defaultDB.settings.paperWidth),
        ...
        for (let i = 0; i < parsed.events.length; i++) {
          ...
          if (!e.id || typeof e.id !== 'string') {
            return reject(new Error(`Malformed backup: Event at index ${i} is missing a valid id.`));
          }
          if (!e.eventName || typeof e.eventName !== 'string') {
            return reject(new Error(`Malformed backup: Event '${e.id}' is missing a valid eventName.`));
          }
          if (e.totalAmount !== undefined && typeof e.totalAmount !== 'number') {
            return reject(new Error(`Malformed backup: Event '${e.id}' has an invalid non-numeric totalAmount.`));
          }
          if (e.totalEntries !== undefined && typeof e.totalEntries !== 'number') {
            return reject(new Error(`Malformed backup: Event '${e.id}' has an invalid non-numeric totalEntries.`));
          }
        ...
        for (let i = 0; i < parsed.entries.length; i++) {
          ...
          if (!entry.id || typeof entry.id !== 'string') {
            return reject(new Error(`Malformed backup: Entry at index ${i} is missing a valid id.`));
          }
          if (!entry.eventId || typeof entry.eventId !== 'string') {
            return reject(new Error(`Malformed backup: Entry '${entry.id}' is missing eventId.`));
          }
          if (!entry.name || typeof entry.name !== 'string') {
            return reject(new Error(`Malformed backup: Entry '${entry.id}' is missing a guest name.`));
          }
          if (entry.amount === undefined || typeof entry.amount !== 'number') {
            return reject(new Error(`Malformed backup: Entry '${entry.id}' must contain a numeric amount.`));
          }
          if (entry.amount <= 0) {
            return reject(new Error(`Malformed backup: Entry '${entry.id}' has an invalid non-positive amount.`));
          }
