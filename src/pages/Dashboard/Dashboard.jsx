...
import { Calendar, Users, IndianRupee, TrendingUp, Plus, ArrowRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      setIsLoading(true);
      try {
        const [loadedEvents, loadedEntries, settings] = await Promise.all([
  ...
  const { totalEvents, totalEntries, totalCollection, todaysCollection } = useTotals(events, entries);

  const stats = useMemo(() => [
    { title: 'Total Events', value: String(totalEvents), icon: Calendar, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30' },
    { title: 'Total Entries', value: totalEntries.toLocaleString('en-IN'), icon: Users, color: 'text-indigo-500', bg: 'bg-indigo-100 dark:bg-indigo-900/30' },
    { title: 'Total Collection', value: `${currency} ${totalCollection.toLocaleString('en-IN')}`, icon: IndianRupee, color: 'text-emerald-500', bg: 'bg-emerald-100 dark:bg-emerald-900/30' },
    { title: "Today's Collection", value: `${currency} ${todaysCollection.toLocaleString('en-IN')}`, icon: TrendingUp, color: 'text-amber-500', bg: 'bg-amber-100 dark:bg-amber-900/30' },
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p style={{fontFamily:'"Lemon",Aerial'}} className="text-gray-500 dark:text-gray-400 mt-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
        {isLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <Card key={i} className="border-0 shadow-sm ring-1 ring-gray-200 dark:ring-gray-800">
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-12 text-center text-gray-500">
              <p className="text-sm">No events found. Start by creating a new event!</p>
              <Button 
                variant="outline" 
                size="sm" 
                className="mt-4" 
                ...
                {recentEvents.map((event) => (
                  ...
                  >
                    ...
                    <TableCell className="text-right pr-6">
                      <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(ROUTES.CURRENT_EVENT, { state: { eventId: event.id } }); }}>
                        Open
                      </Button>
                    </TableCell>
                  </TableRow>
